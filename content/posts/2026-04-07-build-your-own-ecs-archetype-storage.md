...
The complete file is around 250 lines and lives as a [gist](https://gist.github.com/matthewjberger/bbf7e0dc3fd5be28fe01f5c72cdaedb7). Run `cargo new ecs && cd ecs`, replace `src/main.rs` with its contents, then `cargo run`. It should print the mover's state and then `ok`. The final block of `main` exercises the generational guard. After we despawn `mover`, the next spawn reuses its id but bumps the generation, and the original handle no longer resolves.
